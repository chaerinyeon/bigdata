...
이 기준을 바탕으로 국가·산업 단위의 위험을 측정하고, 이를 입력으로 몬테카를로 시뮬레이션을 실행하여 **위험 대비 기대 성공률이 가장 높은 자본 배분**를 탐색합니다.
...
- 필요 자본은 [5-4]의 **중앙값**을 사용해 현실적인 투자 가능 기업 수를 산출
...
- **데이터에 밸류에이션·지분 희석·투자 시점 정보가 없었기 때문에**, 성과를 IRR이 아니라 '폐업/성공 엑시트 여부'로 **단순화할 수밖에 없었습니다.** → 실제 수익률·진입/회수 타이밍 관점의 결론은 데이터 보강 후 재검증이 필요합니다.
- **`category_list`가 멀티태그였기 때문에** Biotech 성공률이 소수 표본(`Biotechnology|Health Care`, n=110)에서 **89%로 과대평가**되었고(단독 `Biotechnology` n=3,432는 16.1%), 이 값이 시뮬레이션에 그대로 들어가 **"Biotech 집중" 결론이 과장**되었습니다. → 따라서 실제 적용 전 **첫 태그 정규화 + 최소 표본 임계치(N≥200)** 로 성공률을 재산출해야 하며, 그 전까지 "Biotech 단독 집중 투자"는 그대로 받아들이면 안 됩니다.
- 리스크 점수가 휴리스틱 정의이고 NoSQL·실시간 처리를 제외했기 때문에, 향후 **Markowitz 분산 기반 위험모형**과 **생존분석·스트리밍**을 더하면 "어디에·얼마나"를 넘어 **"언제" 투자할지**까지 확장할 수 있습니다.
